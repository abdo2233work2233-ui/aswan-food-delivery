  ...
  const onProfileSubmit = async (data: ProfileFormData) => {
    try {
      ...
      if (updateProfile.fulfilled.match(result)) {
        ...
        toast.error((result.payload as string) || (isArabic ? 'فشل في تحديث الملف الشخصي' : 'Failed to update profile'));
      }
    } catch (error) {
  ...
  const handleSetDefaultAddress = async (addressId: string) => {
    try {
      ...
      if (setDefaultAddress.fulfilled.match(result)) {
        ...
        toast.error((result.payload as string) || (isArabic ? 'فشل في تعيين العنوان الافتراضي' : 'Failed to set default address'));
      }
    } catch (error) {
      toast.error(isArabic ? 'حدث خطأ غير متوقع' : 'An unexpected error occurred');
    }
