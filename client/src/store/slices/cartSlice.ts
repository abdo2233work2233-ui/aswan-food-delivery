...
import { calculateDeliveryFee, calculateTax, calculateTotal, getRestaurantsFromCart } from '../../utils/cartCalculations';
...
const loadCartFromStorage = (): Cart => {
  try {
    const savedCart = localStorage.getItem('aswan-food-cart');
    if (savedCart) {
...
const saveCartToStorage = (cart: Cart) => {
  try {
    localStorage.setItem('aswan-food-cart', JSON.stringify(cart));
  } catch (error) {
  ...
  reducers: {
    addToCart: (state, action: PayloadAction<{ menuItem: MenuItem; quantity?: number; notes?: string }>) => {
      ...
      if (!state.cart.restaurantId) {
        ...
        // Create a basic restaurant object from the restaurantId
      ...
      // Recalculate totals
      const restaurants = getRestaurantsFromCart(state.cart);
      const deliveryFee = calculateDeliveryFee(state.cart.subtotal, restaurants);
      const tax = calculateTax(state.cart.subtotal, deliveryFee, state.cart.discount);
      const total = calculateTotal(state.cart.subtotal, deliveryFee, tax, state.cart.discount);
    ...
    clearCart: (state) => {
      ...
      localStorage.removeItem('aswan-food-cart');
    },
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(calculateOrderTotal.rejected, (state, action) => {
        state.isCalculating = false;
        state.error = (action.payload as string) || 'Failed to calculate order total';
      });
  },
});
