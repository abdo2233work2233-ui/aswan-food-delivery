...
>(
  ...
  async (filters = {}, { rejectWithValue }) => {
    try {
      const response = await restaurantService.getRestaurants(filters as RestaurantFilters);
      return response!;
    } catch (error: any) {
...
>(
  ...
  async (limit = 20, { rejectWithValue }) => {
    try {
      const response = await restaurantService.getPopularItems(limit as number);
      return response.data!;
    } catch (error: any) {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchPopularItems.rejected, (state, action) => {
        state.isLoading = false;
        state.error = (action.payload as string) || 'Failed to fetch popular items';
      });
