  ...
  const handleAddToCart = async () => {
    ...
    if (cart.restaurantId && cart.restaurantId !== menuItem.restaurantId) {
      const confirmMessage = isArabic 
        ? `لديك عناصر من مطعم آخر في السلة. هل تريد إفراغ السلة وإضافة هذا العنصر؟`
      ...
      <div className="flex">
        ...
        <div className="w-24 h-24 sm:w-32 sm:h-32 bg-neutral-200 overflow-hidden flex-shrink-0">
          <Image
            ...
            fallbackText={isArabic ? menuItem.nameAr.charAt(0) : menuItem.name.charAt(0)}
          />
        ...
        >
          ...
          <div className="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <button
              ...
              className={`btn btn-primary btn-sm flex-1 flex items-center justify-center space-x-2 rtl:space-x-reverse ${
                !menuItem.isAvailable || isAddingToCart ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
              {isAddingToCart ? (
                <>
